...
function numerical_ilu_k!(
    ...
    # IMPORTANT: Save the original matrix values before we start modifying
    # After fill_symbolic!, L has lower triangle and U has upper triangle of A
    A_dense = L_dense + U_dense
    # Fix the diagonal (it's duplicated)
    for i = 1:n
        A_dense[i,i] = U_dense[i,i]  # Use diagonal from U
    end

    # Apply diagonal shift to A
    if shift != 0
        for j = 1:n
            A_dense[j,j] += shift
        end
    ...
    # Initialize L and U for factorization
    L_dense .= T(0)
    U_dense .= T(0)
    ...
    for j = 1:n
        ...
        U_dense[j,j] = A_dense[j,j] - sum_diag
        ...
        for i = j+1:n
            if A_dense[i,j] != 0  # Only if in pattern
                sum_L = T(0)
                for k = 1:j-1
                    sum_L += L_dense[i,k] * U_dense[k,j]
                end
                L_dense[i,j] = (A_dense[i,j] - sum_L) / U_dense[j,j]
            end
        end
        ...
        for i = j+1:n
            if A_dense[j,i] != 0  # Only if in pattern
                sum_U = T(0)
                for k = 1:j-1
                    sum_U += L_dense[j,k] * U_dense[k,i]
                end
                U_dense[j,i] = A_dense[j,i] - sum_U
            end
        end
